...
class NotchOverlayController: NSObject {
    ...
    private func showOverlay(
        ...
        self.onNextPage = isPreviewOnly ? nil : { [weak self] in
            CueRecordService.shared.advanceToNextPage()
        }
        ...
        if settings.overlayMode == .fullscreen {
            ...
            showFullscreen(settings: settings, screen: fsScreen, installsKeyMonitor: !isPreviewOnly)
        } else if settings.overlayMode == .floating && settings.followCursorWhenUndocked {
            showFollowCursor(settings: settings, screen: screen, installsKeyMonitor: !isPreviewOnly)
        } else {
            switch settings.overlayMode {
            case .pinned:
                showPinned(settings: settings, screen: screen, installsKeyMonitor: !isPreviewOnly)
            case .floating:
                showFloating(settings: settings, screenFrame: screenFrame, installsKeyMonitor: !isPreviewOnly)
            case .fullscreen:
                break // handled above
            }
        }
    ...
    private func showPinned(settings: NotchSettings, screen: NSScreen, installsKeyMonitor: Bool = true) {
    ...
    private func showFollowCursor(settings: NotchSettings, screen: NSScreen, installsKeyMonitor: Bool = true) {
    ...
    private func showFullscreen(settings: NotchSettings, screen: NSScreen, installsKeyMonitor: Bool = true) {
        let screenFrame = screen.frame
    ...
    private func showFloating(settings: NotchSettings, screenFrame: CGRect, installsKeyMonitor: Bool = true) {
        let panelWidth = settings.notchWidth
        let panelHeight = settings.textAreaHeight
        ...
        Timer.publish(every: 0.1, on: .main, in: .common)
            ...
            .sink { [weak self] _ in
                ...
                if let targetIndex = self.overlayContent.jumpToPageIndex {
                    ...
                    CueRecordService.shared.jumpToPage(index: targetIndex)
                }
